...
class OP_Printer{
    ...
    public function __construct()
    {
        ...
        add_action( 'wp_ajax_op_print_setting', array($this,'save_setting') );
    ...
    public function pos_admin_menu(){
        ...
        $page = add_submenu_page( 'openpos-dasboard', __( 'Star CloudPRNT', 'woo-book-price' ),  __( 'Star CloudPRNT', 'woo-book-price' ) , 'manage_woocommerce', 'op-cloudrrnt', array( $this, 'setting_page' ) );
    ...
    public function admin_enqueue(){
        ...
        wp_enqueue_style('op-printer-bootstrap', OPENPOS_CLOUDPRNT_URL.'/assets/css/bootstrap.css');

        wp_enqueue_style('openpos-printer.admin', OPENPOS_CLOUDPRNT_URL.'/assets/css/admin.css',array('op-printer-bootstrap'));

        wp_enqueue_script('openpos-printer.admin.bootstrap', OPENPOS_CLOUDPRNT_URL.'/assets/js/bootstrap.js',array('jquery'));

    }
    public function setting_page(){
        $setting = $this->get_setting();
        $printers = $this->getPrinters();
        require(OPENPOS_CLOUDPRNT_DIR.'templates/setting.php');
    }

    public function save_setting(){
        $setting = isset($_REQUEST['_op_star_cloud_prnt']) ? $_REQUEST['_op_star_cloud_prnt'] : array();
        update_option( '_op_star_cloudprnt_setting', json_encode($setting) );
        $result = array(
            'status' => '1',
    ...
    public function get_setting(){
        ...
        $setting_json = get_option('_op_star_cloudprnt_setting',json_encode($default));
            ...
            if($_FILES){
                foreach($_FILES as $file_key => $file)
                {
                    if($file_key == 'file_html')
                    {
                        ...
                        file_put_contents(rtrim(OPENPOS_CLOUDPRNT_DIR,'/').'/files/'.time().'_receipt.html',$html);
                        ...
                        if($type == 'image/png')
                        {
                            $target_file = rtrim(OPENPOS_CLOUDPRNT_DIR,'/').'/files/'.time().'_'.$file["name"];
    ...
    public function getPrinters(){
        $list = star_cloudprnt_get_printer_list();
        return $list;
    }
    ...
    public function op_register_form_end($default,$warehouses,$cashiers){
        ...
        <div class="form-group">
            ...
            <div class="col-sm-10">
                <select class="form-control" name="_op_register_star_cloudprnt">
